...
async def _help(c: UtubeBot, m: Message):
    ...
    help_text = (
        ...
        "7️⃣ Use `/setprefix` or `/sp` to set Prefix.\n"
        "8️⃣ Use `/setsuffix` or `/ss` to set Suffix.\n"
        "8️⃣ Use `/setplaylist` or `/spid` to manually set Playlist ID.\n"
        "9️⃣ Use `/showsettings` or `/sst` to To See the Newly Added or Updated Settings.\n"
        "🔟 Use `/resetsettings` or `/rst` to To Reset the Settings.\n\n"
